...
#include "network/eblacap/packets_handlers/latest/common/base_packet_handler.hpp"
...
struct EblacapTpTest : NodesTest {};
...
struct HandlersInitData {
  ...
  std::shared_ptr<eblacap::PeersState> peers_state;
  std::shared_ptr<eblacap::TimePeriodPacketsStats> packets_stats;
...
class DummyPacketHandler : public network::eblacap::BasePacketHandler {
 public:
 ...
 private:
  void process(DummyPacket&& packet, [[maybe_unused]] const std::shared_ptr<eblacap::EblaPeer>& peer) {
...
HandlersInitData createHandlersInitData() {
  ...
  ret_init_data.peers_state = std::make_shared<eblacap::PeersState>(std::weak_ptr<dev::p2p::Host>(), FullNodeConfig());
  ret_init_data.packets_stats =
      std::make_shared<eblacap::TimePeriodPacketsStats>(std::chrono::milliseconds(0), ret_init_data.own_node_addr);
...
std::pair<eblacap::EblacapVersion, threadpool::PacketData> createPacket(
...
TEST_F(EblacapTpTest, block_free_packets) {
  ...
  auto packets_handler = std::make_shared<eblacap::PacketsHandler>();
...
TEST_F(EblacapTpTest, hard_blocking_deps) {
  HandlersInitData init_data = createHandlersInitData();
...
TEST_F(EblacapTpTest, peer_order_blocking_deps) {
  HandlersInitData init_data = createHandlersInitData();
...
TEST_F(EblacapTpTest, same_dag_blks_ordering) {
  HandlersInitData init_data = createHandlersInitData();
...
TEST_F(EblacapTpTest, dag_blks_lvls_ordering) {
  HandlersInitData init_data = createHandlersInitData();
...
TEST_F(EblacapTpTest, threads_borrowing) {
  HandlersInitData init_data = createHandlersInitData();
...
TEST_F(EblacapTpTest, low_priotity_queue_starvation) {
  HandlersInitData init_data = createHandlersInitData();
