...
Query::Query(std::shared_ptr<::ebla::final_chain::FinalChain> final_chain,
    ...
    : final_chain_(std::move(final_chain)),
      ...
      kChainId(chain_id) {
  get_block_by_num_ = [&](::ebla::EthBlockNumber num) {
...
std::shared_ptr<object::Block> Query::getBlock(std::optional<response::Value>&& number,
  ...
  std::optional<::ebla::EthBlockNumber> block_number;
  if (number) {
  ...
  if (block_number == 0) [[unlikely]] {
    return std::make_shared<object::Block>(std::make_shared<Block>(
        final_chain_, transaction_manager_, get_block_by_num_, ::ebla::blk_hash_t(), block_header));
  }
...
std::shared_ptr<object::Transaction> Query::getTransaction(response::Value&& hashArg) const {
  if (auto transaction = transaction_manager_->getTransaction(::ebla::trx_hash_t(hashArg.get<std::string>()))) {
...
std::shared_ptr<object::Account> Query::getAccount(response::Value&& addressArg,
  ...
  const auto address = ::ebla::addr_t(addressArg.get<std::string>());
  if (blockArg) {
...
std::shared_ptr<object::DagBlock> Query::getDagBlock(std::optional<response::Value>&& hashArg) const {
  ...
  if (hashArg) {
    ...
    if (dag_blocks.size() > 0) {
      ebla_dag_block = dag_blocks.front();
    }
  }
  if (ebla_dag_block) {
    return std::make_shared<object::DagBlock>(std::make_shared<DagBlock>(
        std::move(ebla_dag_block), final_chain_, pbft_manager_, transaction_manager_, get_block_by_num_));
  }
  return nullptr;
}
...
std::vector<std::shared_ptr<object::DagBlock>> Query::getDagBlocks(std::optional<response::Value>&& dagLevelArg,
  ...
  ::ebla::level_t act_dag_level = dag_manager_->getMaxLevel();
