 ...
 **/
std::optional<uint64_t> PacketsThreadPool::push(std::pair<eblacap::EblacapVersion, PacketData>&& packet_data) {
  if (stopProcessing_) {
...
void PacketsThreadPool::processPacket(size_t worker_id) {
  ...
  std::optional<std::pair<eblacap::EblacapVersion, PacketData>> packet;
  ...
  while (stopProcessing_ == false) {
    ...
    LOG(log_dg_) << "Worker (" << worker_id << ") process packet: " << packet->second.type_str_
                 << ", id: " << packet->second.id_ << ", eblacap version: " << packet->first;

    queue_.updateDependenciesStart(packet->second);
    lock.unlock();

    try {
      // Get packets handler based on eblacap version
      ...
      if (packets_handler == packets_handlers_.end()) {
        LOG(log_er_) << "Worker (" << worker_id << ") process packet: " << packet->second.type_str_
                     << ", id: " << packet->second.id_ << ", eblacap version: " << packet->first
                     << " error: Unsupported eblacap version !";
        assert(false);
        throw std::runtime_error("Unsupported eblacap version " + std::to_string(packet->first));
      }
...
void PacketsThreadPool::setPacketsHandlers(eblacap::EblacapVersion eblacap_version,
                                           std::shared_ptr<eblacap::PacketsHandler> packets_handlers) {
  if (!packets_handlers_.emplace(eblacap_version, std::move(packets_handlers)).second) {
    LOG(log_er_) << "Packets handler for capability version " << eblacap_version << " already set";
    assert(false);
  }
}
