...
#include "network/eblacap/packets_handlers/latest/dag_sync_packet_handler.hpp"
...
#include "network/eblacap/packets_handlers/latest/pbft_blocks_bundle_packet_handler.hpp"
...
#include "network/eblacap/packets_handlers/v4/get_pbft_sync_packet_handler.hpp"
...
EblaCapability::EblaCapability(
    EblacapVersion version, const FullNodeConfig &conf, const h256 &genesis_hash, std::weak_ptr<dev::p2p::Host> host,
  ...
  LOG_OBJECTS_CREATE(logs_prefix + "EBLACAP");
  ...
  packets_handlers_ = init_packets_handlers(logs_prefix, conf, genesis_hash, peers_state_, pbft_syncing_state_,
                                            ...
                                            slashing_manager, final_chain, version, node_addr);
...
EblacapVersion EblaCapability::version() const { return version_; }
...
const EblaCapability::InitPacketsHandlers EblaCapability::kInitLatestVersionHandlers =
    [](const std::string &logs_prefix, const FullNodeConfig &config, const h256 &genesis_hash,
       ...
       const std::shared_ptr<final_chain::FinalChain> &final_chain, EblacapVersion, const addr_t &node_addr) {
