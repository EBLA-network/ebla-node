...
void NodeStats::logNodeStats(const std::vector<std::shared_ptr<network::eblacap::EblaPeer>> &all_peers,
...
Json::Value NodeStats::getStatus(
    std::map<network::eblacap::EblacapVersion, std::shared_ptr<network::eblacap::EblaPeer>> peers) const {
  Json::Value res;
