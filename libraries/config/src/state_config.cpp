...
void Config::rlp_without_protocol(util::RLPEncoderRef encoding) const {
...
void append_json(Json::Value& json, const Config& obj) {
  ...
  json["protocol"] = enc_json(obj.protocol);
...
void dec_json(const Json::Value& json, Config& obj) {
  ...
  dec_json(json["protocol"], obj.protocol);
...
RLP_FIELDS_DEFINE(DPOSConfig, eligibility_balance_threshold, vote_eligibility_balance_step, validator_maximum_stake,
                  ...
                  trx_min_gas_price, trx_max_gas_limit, initial_validators)
RLP_FIELDS_DEFINE(Config, evm_chain_config, initial_balances, dpos, protocol)
